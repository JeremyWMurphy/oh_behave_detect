...
while f.UserData.state ~= 3
    ...
    elseif f.UserData.state == 1 % detection or pairing run
        ...
        print_parameters(data_fid_notes,teensy_fs, baseln, n_trials, prcnt_go, sig_amps, prcnt_amps,iti_len,time_out_len, play_error_sound, play_hit_sound, pulse_type, pulse_len, pulse_intrvl, sound_fs, err_freq1, err_freq2, err_amp, err_len, hit_freq1, hit_freq2, hit_amp, hit_len,tp);

        %% setup trial parameter distributions       
        % iti_dist = iti_sd.*randn(n_trials,1) + iti_mu;
        iti_dist = iti_len(1) + ((iti_len(2) - iti_len(1)) * rand(n_trials,1));
        ...
        while present % trial loop
            ...
            if prior_was_error
                iti = round(1e3*(time_out_len(1) + (time_out_len(2) - time_out_len(1)) * rand));
                prior_was_error = false;                
            else
